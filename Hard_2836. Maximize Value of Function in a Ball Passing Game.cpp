...
public:
    long long getMaxFunctionValue(vector<int>& receiver, long long k) 
    {
        ...
        {            
            for(int x=0; x<n; ++x)
            {                
                destination[i][x] = destination[i-1][destination[i-1][x]];
                value[i][x] = value[i-1][x] + value[i-1][destination[i-1][x]] - destination[i-1][x];
            }
        }
        ...
        */
        ...
        vector<int> final_destination(n);
        vector<long long> final_value(n);
        for(int i=0; i<n; ++i)
        {
        ...
        for(int i=0; i<cnt; ++i)
        {
            ...
            for(int x=0; x<n; ++x)
            {
                final_value[x] = final_value[x] + value[i][final_destination[x]] - final_destination[x];
                final_destination[x] = destination[i][final_destination[x]];                
            }
            //cout<<endl;
        }
                
        return *max_element(final_value.begin(), final_value.end());
    }
};
